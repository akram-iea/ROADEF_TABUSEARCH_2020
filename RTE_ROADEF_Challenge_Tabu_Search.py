...
def compute_risk_distribution(Interventions: dict, T_max: int, scenario_numbers):
    ...
    for intervention in Interventions.values():
        ...
        for time in range(start_time_idx, start_time_idx + delta):
            for i, additional_risk in enumerate(
                intervention_risk[str(time + 1)][str(start_time)]
            ):
...
def check_schedule(instance: dict):
    ...
    for intervention_name, intervention in interventions.items():
        ...
        if not START_STR in intervention:
            print(
                "ERROR: Schedule constraint 4.1.2: Intervention "
                + intervention_name
        ...
        if not (1 <= start_time <= horizon_end):
            print(
                "ERROR: Schedule constraint 4.1 time validity: Intervention "
                + intervention_name
                + " starting time "
                + str(start_time)
                + " is not a valid starting date. Expected value between 1 and "
                + str(horizon_end)
                + "."
            )
        ...
        if time_limit < start_time:
            print(
                "ERROR: Schedule constraint 4.1.3: Intervention "
                + intervention_name
                + " realization exceeds time limit."
                + " It starts at "
                + str(start_time)
                + " while time limit is "
                + str(time_limit)
    ...
    for resource_name, resource in resources.items():
        for time in range(t_max):
            ...
            if worload > upper_bound + tolerance:
                print(
                    "ERROR: resources constraint 4.2 upper bound: Worload on Resource "
                    + resource_name
                    + " at time "
                    + str(time + 1)
                    + " exceeds upper bound."
            ...
            if worload < lower_bound - tolerance:
                print(
                    "ERROR: resources constraint 4.2 lower bound: Worload on Resource "
                    + resource_name
                    + " at time "
                    + str(time + 1)
                    + " does not match lower bound."
                    + " Value "
                    + str(worload)
                    + " is lower than bound "
                    + str(lower_bound)
...
def check_exclusions(Instance: dict):
    ...
    for exclusion in exclusions.values():
        ...
        intervention_1_delta = int(
            intervention_1[DELTA_STR][intervention_1_start_time - 1]
        )  # get index in list
        intervention_2_delta = int(
            intervention_2[DELTA_STR][intervention_2_start_time - 1]
        )  # get index in list
        # Check overlaps for each time step of the season
        for time_str in Instance[SEASONS_STR][season]:
            time = int(time_str)
            if (
                intervention_1_start_time
                <= time
                < intervention_1_start_time + intervention_1_delta
            ) and (
                intervention_2_start_time
                <= time
                < intervention_2_start_time + intervention_2_delta
            ):
                print(
                    "ERROR: exclusions constraint 4.3: interventions "
                    + intervention_1_name
                    + " and "
                    + intervention_2_name
                    + " are both ongoing at time "
                    + str(time)
                    + "."
                )
                return False
...
def neighboroperator(solution_file):
    ...
    for j in interventions_list:
        for h in range(len(interventions_list)):
            instance["Interventions"][interventions_list[h]]["start"] = start_position[
                h
            ]
        for i in range(1, 19):
            ...
            if check_all_constraints(instance):
                Neighborhood.append(
                    [instance["Interventions"][u]["start"] for u in interventions_list]
                )
...
class TabuSearch:
    ...
    def isTerminationCriteriaMet(self):
        ...
        return (
            self.evaluate(self.bestSolution) < self.acceptableScoreThreshold
